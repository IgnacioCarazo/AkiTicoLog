...
% famositico(nombre,edad,lugar de nacimiento, color de pelo, estatura,
% profesion, color de ojos)
% ##############POR AHORA SOLO ESTAN CORRECTOS LOS DATOS DE HANNAH, LOS
% DEMAS SON PARA PROBAR.###############
famositico("Hanna Gabriels","35","alajuela","negro","1.87m","boxeadora","verdes","mujer").
famositico("Keylor Navas","34","cartago","cafe","1.80m","boxeadora","cafes","hombre").
famositico("Choche Romano","30","sanjose","cafe","1.78m","comediante","negros","hombre").
famositico("Edgar Silva","35","heredia","negro","1.70m","periodista","cafes","hombre").
...
buscar_data("mide", L, Estatura) :-
    famositico(_,_,_,_,Estatura,_,_,_),
    buscar_palabra(Estatura, L),
    !.

buscar_data("es", L, Profesion) :-
    famositico(_,_,_,_,_,Profesion,_,_),
...
verificar_datos(L, Palabra, Atributo) :-
    ...
    write("La base de datos no presenta a nadie con esas caracteristicas, vuelva a intentar"),!.
...
pregunta1() :-
    ...
    write_ln(Palabra),
    write_ln(Atributo),
    famositico(S,_,_,_,_,_,_,Atributo),
    write(S).
